...
pub fn exec_invokestatic(frame: &mut Frame, code: &[u8], vm: Option<&mut crate::vm::Vm>) -> Result<(), JvmError> {
    ...
    match &cp[index - 1] {
        reader::constant_pool::CpInfo::MethodRef { class_index, name_and_type_index, .. } => {
            let class_name = if let reader::constant_pool::CpInfo::Class { name_index, .. } = &cp[*class_index as usize - 1] {
                cp.get_utf8_string(*name_index)
            } else {
                return Err(JvmError::IllegalStateError("Invalid class reference in MethodRef".to_string()));
            };
